...
export default () => {
    ...
    const variableRef = useRef<{
        open: (type: "add" | "edit", variableType: "project" | "page", variable?: PageVariable) => void;
    }>();
    // 页面组件
    ...
    const columns = (variableType: "project" | "page"): TableProps<PageVariable>["columns"] => [
        ...
        },
        {
            ...
            render: (_, row) => (
                <div className={styles.iconCol}>
                    {row.type === "array" || row.type === "object" ? row.type : row.defaultValue}
                </div>
            ),
        },
        {
        ...
        },
        {
            ...
            render: (_, row) => (
                <div style={{ display: "flex", alignItems: "center" }}>
                    <Button
                        type="link"
                        onClick={(event) => handleEdit(event, row, variableType)}
                        style={{ paddingLeft: 0 }}
                    >
                        修改
                    </Button>
