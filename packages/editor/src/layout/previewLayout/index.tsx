...
import { getMenuList } from '@/invokeApi';
import projectApi from '@/invokeApi/project';
import Tab from './components/Tab';
import Logo from './components/Logo/Logo';
import BreadList from './components/BreadList/BreadList';
import { arrayToTree, newArrayToTree } from '@/utils/util';
...
const AdminLayout = () => {
  ...
  useEffect(() => {
    ...
    const fetchProjectDetail = async () => {
      if (projectId) {
        const detail = await projectApi.getProjectDetail(projectId).catch(() => {
        ...
        const menus = await getMenuList({ projectId, status: -1 }).catch(() => {
          return navigate('/403?type=project');
        });
        if (!menus) return;
        ...
        const { menuTree, buttons, pageMap, menuMap } = newArrayToTree(menus);
