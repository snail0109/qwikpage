...
fn generate_template(element: &MergedElement) -> String {
    if element.type_name == "Loop" {
        ...
        return format!(r#"<q-flex v-for="{}">{}</q-flex>"#, loop_expr, children);
    }
    ...
    let config_js = value_to_js_object_literal(&element.config);
