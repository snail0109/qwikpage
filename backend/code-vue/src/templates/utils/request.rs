...
instance.interceptors.request.use((config) => {
  ...
  const {
    ...
    requestInterceptor,
  } = pageStore.pageState.page.pageData.interceptor || {};
  ...
  // 接口跨域转发
  if (config.isCors && config.url && config.url.indexOf('http') === 0) {
    const urlObj = new URL(config.url);
    const path = urlObj.pathname + urlObj.search + urlObj.hash;
    config.url = `${import.meta.env.VITE_BASE_API}${path}`;
  }
...
instance.interceptors.response.use(
  (response) => {
    const pageStore = usePageStore();
    const { responseInterceptor } = pageStore.pageState.page.pageData.interceptor;
    // 返回拦截
    let res = response;
