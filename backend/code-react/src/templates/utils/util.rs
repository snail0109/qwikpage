...
export function getPageVariable(name?: string) {
  ...
  pageData.variables.forEach((item: any) => {
    data[item.name] = pageData.variableData[item.name] ?? item.defaultValue;
  });
...
export const dateFormat = (list: Array<ComponentType>, values: any) => {
  const pageStore = usePageStore();
  const elementsMap = pageStore.pageState.page.pageData.elementsMap;
